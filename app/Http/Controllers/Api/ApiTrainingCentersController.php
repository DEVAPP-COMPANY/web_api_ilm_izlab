...
class ApiTrainingCentersController extends Controller
{
    ...
    public function getFilterKeyword($centers, $keyword){
        $centers = $centers
                ->where('training_centers.name', 'LIKE', "%{$keyword}%")
                ->orWhere('training_centers.comment', 'LIKE', "%{$keyword}%")
    ...
    public function centerDetail($center_id){
        $center = TrainingCenter::where('training_centers.id', $center_id)
            ...
            ->first();
                ...
                $subscriber = $center->subsriptions ?? [];
                if($subscriber != []){
                    $subscriber_count = count($center->subsriptions);
                    $center['subsribers_count'] = $subscriber_count;
                    unset($center->subsriptions);
        ...
        return $this->sendResponse($center, true, "");
